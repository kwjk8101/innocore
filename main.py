...
def load_crypto_fg():
    """
    Crypto Fear & Greed from Alternative.me — the industry standard free source,
    referenced by CoinMarketCap and CoinGecko. No API key needed.
    """
    try:
        r = requests.get(
        ...
        current = data[0]
        history = [
            (datetime.fromtimestamp(int(d["timestamp"])), int(d["value"]), d["value_classification"])
            for d in data
        ]
...
def load_stock_fg():
    """
    Stock Fear & Greed from CNN Business public API.
    Tries multiple User-Agent strings for reliability on cloud servers.
    Fails gracefully — returns None so the rest of the app still loads.
    """
    ua_list = [
    ...
    for ua in ua_list:
        try:
            ...
            return {"value": round(score, 1), "label": label, "history": history}
        except Exception:
            continue
    return {"value": None, "label": "Unavailable", "history": []}
...
# ── API key check (server-side only — never shown to users) ──────────────────
...
with s3:
    ...
    st.markdown(
        ...
        f'<div class="sentiment-val" style="color:{vc}">{f"{vv:.1f}" if vv is not None else "—"}</div>'
