...
# ── Market session schedule — LOCAL exchange hours ───────────────────────────
# All open/close times are in the exchange's OWN local timezone.
# SGX 09:00-12:30, 14:00-17:00 SGT (lunch break 12:30-14:00)
# Tokyo 09:00-11:30, 12:30-15:30 JST (lunch break 11:30-12:30)
# Hong Kong 09:30-12:00, 13:00-16:00 HKT (lunch break 12:00-13:00)
MARKET_SESSIONS = {
    "SGX":       {"open": (9, 0),  "close": (17, 0),  "break": ((12,30),(14, 0)), "color": "#22d3ee", "tz": "Asia/Singapore"},
    "Tokyo":     {"open": (9, 0),  "close": (15, 30), "break": ((11,30),(12,30)), "color": "#f59e0b", "tz": "Asia/Tokyo"},
    "London":    {"open": (8, 0),  "close": (16, 30), "break": None,             "color": "#a78bfa", "tz": "Europe/London"},
    "New York":  {"open": (9, 30), "close": (16, 0),  "break": None,             "color": "#f87171", "tz": "America/New_York"},
    "Frankfurt": {"open": (9, 0),  "close": (17, 30), "break": None,             "color": "#86efac", "tz": "Europe/Berlin"},
    "Hong Kong": {"open": (9, 30), "close": (16, 0),  "break": ((12, 0),(13, 0)),"color": "#fb923c", "tz": "Asia/Hong_Kong"},
}
...
def load_market_breadth():
    """
    S&P 500 market breadth using the full index constituent list (503 tickers).
    Uses yf.download() batch to avoid per-ticker rate limits.
    Computes % of stocks above their 50-day and 200-day moving averages.
    TTL is 1800s (30 min) — this is a heavy call, cached aggressively.
    """
    # Full S&P 500 constituent list (503 as of 2025)
    SP500 = [
        ...
        "AXON","BKR","BALL","BAC","BK","BBWI","BAX","BDX","BBY","BIO","TECH","BIIB","BLK","BX",
        "BA","BKNG","BWA","BSX","BMY","AVGO","BR","BRO","BF.B","BLDR","BG","CDNS","CZR","CPT",
        "CPB","COF","CAH","KMX","CCL","CARR","CTLT","CAT","CBOE","CBRE","CDW","CE","COR","CNC",
        "CNP","CF","CRL","SCHW","CHTR","CVX","CMG","CB","CHD","CI","CINF","CTAS","CSCO","C",
        "CFG","CLX","CME","CMS","KO","CTSH","CL","CMCSA","CAG","COP","ED","STZ","CEG","COO",
        "CPRT","GLW","CPAY","CTVA","CSGP","COST","CTRA","CRWD","CCI","CSX","CMI","CVS","DHI",
        "DHR","DRI","DVA","DAY","DECK","DE","DELL","DAL","DVN","DXCM","FANG","DLR","DFS","DG",
        "DLTR","D","DPZ","DOV","DOW","DHC","DTE","DUK","DD","EMN","ETN","EBAY","ECL","EIX",
        "EW","EA","ELV","LLY","EMR","ENPH","ETR","EOG","EPAM","EQT","EFX","EQIX","EQR","ESS",
        "EL","ETSY","EG","EVRG","ES","EXC","EXPE","EXPD","EXR","XOM","FFIV","FDS","FICO","FAST",
        "FRT","FDX","FIS","FITB","FSLR","FE","FI","FMC","F","FTNT","FTV","FOXA","FOX","BEN",
        "FCX","GRMN","IT","GE","GEHC","GEV","GEN","GNRC","GD","GIS","GM","GPC","GILD","GS",
        "HAL","HIG","HAS","HCA","DOC","HSIC","HSY","HES","HPE","HLT","HOLX","HD","HON","HRL",
        "HST","HWM","HPQ","HUBB","HUM","HBAN","HII","IBM","IEX","IDXX","ITW","INCY","IR","PODD",
        "INTC","ICE","IFF","IP","IPG","INTU","ISRG","IVZ","INVH","IQV","IRM","JBAL","JKHY","J",
        "JBL","JNPR","JPM","JNPR","K","KVUE","KDP","KEY","KEYS","KMB","KIM","KMI","KKR","KLAC",
        "KHC","KR","LHX","LH","LRCX","LW","LVS","LDOS","LEN","LNC","LIN","LYV","LKQ","LMT",
        "L","LOW","LYB","MTB","MRO","MPC","MKTX","MAR","MMC","MLM","MAS","MA","MTCH","MKC","MCD",
        "MCK","MDT","MRK","META","MET","MTD","MGM","MCHP","MU","MSFT","MAA","MRNA","MHK","MOH",
        "TAP","MDLZ","MPWR","MNST","MCO","MS","MOS","MSI","MSCI","NDAQ","NTAP","NFLX","NEM","NWSA",
        "NWS","NEE","NKE","NI","NDSN","NSC","NTRS","NOC","NCLH","NRG","NUE","NVR","NVDA","NVS",
        "ORLY","OXY","ODFL","OMC","ON","OKE","ORCL","OTIS","PCAR","PKG","PANW","PH","PAYX","PAYC",
        ...
        "VLO","VTR","VLTO","VRSN","VRSK","VZ","VRTX","VTRS","VICI","V","VMC","WRB","GWW","WAB",
        "WBA","WMT","WBD","WM","WAT","WEC","WFC","WELL","WST","WDC","WHR","WMB","WTW","WLTW",
        "WYNN","XEL","XYL","YUM","ZBRA","ZBH","ZTS",
    ]
    try:
        raw = yf.download(
            SP500, period="1y", interval="1d",
            group_by="ticker", auto_adjust=True,
    ...
    above_50 = 0; above_200 = 0; valid = 0
    if raw is not None and not raw.empty:
        for ticker in SP500:
            try:
                if isinstance(raw.columns, pd.MultiIndex):
                    if ticker not in raw.columns.get_level_values(0): continue
                    closes = raw[ticker]["Close"].dropna()
                else:
                    closes = raw["Close"].dropna() if "Close" in raw.columns else None
                if closes is None or len(closes) < 200: continue
                now   = float(closes.iloc[-1])
                ...
                pass
    # Fallback: individual calls for at least 50 stocks if batch failed
    if valid < 20:
        FALLBACK = [
            "AAPL","MSFT","AMZN","GOOGL","META","NVDA","TSLA","V","MA","JPM",
            "BAC","GS","WFC","JNJ","UNH","PFE","MRK","XOM","CVX","COP",
            "CAT","HON","GE","WMT","HD","MCD","COST","DIS","NFLX","CMCSA",
            "NEE","DUK","SO","AMT","PLD","LIN","APD","NEM","FCX","T",
            "VZ","CSCO","ORCL","ADBE","CRM","INTU","AMD","QCOM","TXN","AVGO",
        ]
        above_50 = 0; above_200 = 0; valid = 0
        for ticker in FALLBACK:
            try:
                ...
                c = h["Close"].dropna()
                now = float(c.iloc[-1])
                if now > float(c.tail(50).mean()):  above_50  += 1
                if now > float(c.tail(200).mean()): above_200 += 1
                valid += 1
            except Exception:
                pass
    if valid == 0:
...
def get_market_session_status():
    ...
    v16 fix: open/close now stored as LOCAL exchange hours compared against local
    time. Previous version stored UTC hours but compared against local — causing
    all Asian sessions to show wrong status. Also handles lunch breaks.
    """
    ...
    for market, info in MARKET_SESSIONS.items():
        try:
            ...
            in_lunch = False
            if info["break"]:
                bs = info["break"][0][0]*60 + info["break"][0][1]
                be = info["break"][1][0]*60 + info["break"][1][1]
                in_lunch = bs <= cur_mins < be
            in_session = is_weekday and open_mins <= cur_mins < close_mins and not in_lunch
            if in_session:
                status = "OPEN"
                if info["break"]:
                    bs = info["break"][0][0]*60 + info["break"][0][1]
                    if cur_mins < bs:
                        left = bs - cur_mins
                        time_str = f"Lunch in {left}m" if left < 60 else f"Lunch in {left//60}h {left%60}m"
                    else:
                        left = close_mins - cur_mins
                        time_str = f"Closes in {left}m" if left < 60 else f"Closes in {left//60}h {left%60}m"
                else:
                    left = close_mins - cur_mins
                    time_str = f"Closes in {left}m" if left < 60 else f"Closes in {left//60}h {left%60}m"
            elif in_lunch:
                ...
                be = info["break"][1][0]*60 + info["break"][1][1]
                left = be - cur_mins
                time_str = f"Reopens in {left}m"
            else:
                status = "CLOSED"
                if not is_weekday or cur_mins >= close_mins:
                    wd = local_now.weekday()
                    days_ahead = 3 if (wd==4 and cur_mins>=close_mins) else 2 if wd==5 else 1
                    next_open = (local_now + timedelta(days=days_ahead)).replace(
                        hour=open_h, minute=open_m, second=0, microsecond=0)
                else:
                    next_open = local_now.replace(
                        hour=open_h, minute=open_m, second=0, microsecond=0)
                mins_until = int((next_open - local_now).total_seconds() / 60)
                time_str = (f"Opens in {mins_until}m" if mins_until < 60
                            else f"Opens in {mins_until//60}h {mins_until%60}m" if mins_until < 1440
                            else f"Opens {next_open.strftime('%a %H:%M')}")
            # Convert local open/close to SGT for display
            try:
                naive_open  = local_now.replace(hour=open_h,  minute=open_m,  second=0, microsecond=0, tzinfo=None)
                naive_close = local_now.replace(hour=close_h, minute=close_m, second=0, microsecond=0, tzinfo=None)
                sgt_open_str  = local_tz.localize(naive_open).astimezone(sgt_tz).strftime("%H:%M")
                sgt_close_str = local_tz.localize(naive_close).astimezone(sgt_tz).strftime("%H:%M")
            except Exception:
                sgt_open_str  = f"{open_h:02d}:{open_m:02d}"
                sgt_close_str = f"{close_h:02d}:{close_m:02d}"
            statuses[market] = dict(
                status=status, color=info["color"], time_str=time_str,
                sgt_open=sgt_open_str, sgt_close=sgt_close_str,
...
if current_page == "overview":
    ...
    with fg2:
        st.markdown(
            ...
            f'font-family:{MONO};font-size:.52rem;margin-top:3px;text-align:center">'
            f'<span style="color:#4ade80">LOW &lt;20</span>'
            f'<span style="color:#f59e0b">MOD 20-34</span>'
            f'<span style="color:#fb923c">ELEV 35-59</span>'
            f'<span style="color:#f87171">HIGH 60+</span></div>'
            f'<div style="background:#080f1c;border:1px solid #162236;border-radius:6px;'
            f'padding:9px 10px;margin-top:9px">'
            f'<div style="font-family:{MONO};font-size:.59rem;font-weight:700;color:#22d3ee;'
            f'text-transform:uppercase;letter-spacing:.08em;margin-bottom:6px">How this score is built</div>'
            f'<div style="display:grid;gap:3px">'
            f'<div style="display:flex;justify-content:space-between;font-size:.65rem">'
            f'<span style="color:#a0bcd8">Sahm Rule</span>'
            f'<span style="font-family:{MONO};color:#dde8f5">up to 40 pts</span></div>'
            f'<div style="display:flex;justify-content:space-between;font-size:.65rem">'
            f'<span style="color:#a0bcd8">Yield Curve inversion depth</span>'
            f'<span style="font-family:{MONO};color:#dde8f5">up to 25 pts</span></div>'
            f'<div style="display:flex;justify-content:space-between;font-size:.65rem">'
            f'<span style="color:#a0bcd8">VIX stress level</span>'
            f'<span style="font-family:{MONO};color:#dde8f5">up to 15 pts</span></div>'
            f'<div style="display:flex;justify-content:space-between;font-size:.65rem">'
            f'<span style="color:#a0bcd8">HY credit spread widening</span>'
            f'<span style="font-family:{MONO};color:#dde8f5">up to 15 pts</span></div>'
            f'<div style="display:flex;justify-content:space-between;font-size:.65rem">'
            f'<span style="color:#a0bcd8">CFNAI + Real GDP contraction</span>'
            f'<span style="font-family:{MONO};color:#dde8f5">up to 10 pts</span></div>'
            f'</div>'
            f'<div style="margin-top:7px;font-size:.61rem;color:#4a6080;font-style:italic;line-height:1.5">'
            f'A stress indicator, not a forecast. A high score means current conditions resemble '
            f'past pre-recession environments — it does not guarantee a recession will occur.</div>'
            f'</div>'
            f'<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:3px;margin-top:8px">'
            f'<div style="text-align:center;padding:3px;border-radius:3px;background:rgba(74,222,128,.08);font-family:{MONO};font-size:.54rem;color:#4ade80">LOW &lt;20</div>'
            f'<div style="text-align:center;padding:3px;border-radius:3px;background:rgba(245,158,11,.08);font-family:{MONO};font-size:.54rem;color:#f59e0b">MOD 20-34</div>'
            f'<div style="text-align:center;padding:3px;border-radius:3px;background:rgba(251,146,60,.08);font-family:{MONO};font-size:.54rem;color:#fb923c">ELEV 35-59</div>'
            f'<div style="text-align:center;padding:3px;border-radius:3px;background:rgba(248,113,113,.08);font-family:{MONO};font-size:.54rem;color:#f87171">HIGH 60+</div>'
            f'</div>'
            f'</div>', unsafe_allow_html=True)
    ...
    # Five-level graduated scale — avoids binary 0→100 jump
    if score_pct >= 50:
        ts_label, ts_col, ts_bg, ts_icon = "STRONG RISK-ON",  "#4ade80", "rgba(74,222,128,.11)",  "▲▲"
        ts_action = "Strong macro tailwinds across most indicators. Favour cyclicals, growth and commodities. Consider adding equity exposure."
    elif score_pct >= 20:
        ts_label, ts_col, ts_bg, ts_icon = "MILD RISK-ON",    "#86efac", "rgba(134,239,172,.09)", "▲"
        ts_action = "Macro is broadly supportive but not without risks. Lean into equities with selective positioning."
    elif score_pct >= -20:
        ts_label, ts_col, ts_bg, ts_icon = "NEUTRAL",         "#94a3b8", "rgba(148,163,184,.06)", "→"
        ts_action = "Mixed macro signals. Balanced positioning. Avoid concentrated directional bets until clarity improves."
    elif score_pct >= -50:
        ts_label, ts_col, ts_bg, ts_icon = "MILD RISK-OFF",   "#fca5a5", "rgba(252,165,165,.09)", "▼"
        ts_action = "Macro headwinds building. Reduce cyclical exposure, increase defensives. Monitor credit spreads closely."
    else:
        ts_label, ts_col, ts_bg, ts_icon = "STRONG RISK-OFF", "#f87171", "rgba(248,113,113,.11)", "▼▼"
        ts_action = "Broad macro deterioration across most indicators. Defensive posture: healthcare, staples, utilities, cash."
    ...
    st.markdown(
        ...
        f'<div style="display:grid;grid-template-columns:repeat(5,1fr);gap:3px;margin-top:10px">'
        f'<div style="text-align:center;padding:4px 2px;border-radius:3px;background:rgba(74,222,128,.10)"><span style="font-family:{MONO};font-size:.53rem;color:#4ade80;font-weight:700">&#9650;&#9650;<br>Strong<br>Risk-On</span></div>'
        f'<div style="text-align:center;padding:4px 2px;border-radius:3px;background:rgba(134,239,172,.07)"><span style="font-family:{MONO};font-size:.53rem;color:#86efac;font-weight:700">&#9650;<br>Mild<br>Risk-On</span></div>'
        f'<div style="text-align:center;padding:4px 2px;border-radius:3px;background:rgba(148,163,184,.06)"><span style="font-family:{MONO};font-size:.53rem;color:#94a3b8;font-weight:700">&#8594;<br>Neutral</span></div>'
        f'<div style="text-align:center;padding:4px 2px;border-radius:3px;background:rgba(252,165,165,.07)"><span style="font-family:{MONO};font-size:.53rem;color:#fca5a5;font-weight:700">&#9660;<br>Mild<br>Risk-Off</span></div>'
        f'<div style="text-align:center;padding:4px 2px;border-radius:3px;background:rgba(248,113,113,.10)"><span style="font-family:{MONO};font-size:.53rem;color:#f87171;font-weight:700">&#9660;&#9660;<br>Strong<br>Risk-Off</span></div>'
        f'</div>'
        f'<div style="margin-top:8px;font-size:.62rem;color:#2a4a6a;font-family:{MONO}">'
        f'{len(votes)} macro indicators · Score {score_pct:+d}% · Not financial advice</div>'
        f'</div>', unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with op2:
        ...
        # Build enriched P/C display with 5-day trend from opt_hist
        pc_html_str = (
            f'<div style="background:#0c1a2e;border:1px solid #162236;border-radius:8px;padding:13px">'
            f'<div style="font-family:{MONO};font-size:.59rem;font-weight:700;color:#4a7090;'
            f'text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px">'
            f'CBOE Put/Call Ratios — Live</div>'
        )
        pc_rows = [
            ("Equity P/C", "^PCCE", pc_eq,  "PC_EQ",  True,
             "Equity-only options. >0.80 = fear/contrarian buy. <0.55 = complacency warning."),
            ("Total P/C",  "^CPC",  pc_tot, "PC_TOT", False,
             "All CBOE options combined. >1.00 = heavy institutional hedging."),
            ("Index P/C",  "^PCCR", pc_idx, "PC_IDX", False,
             "Index options only. Dominated by institutional hedges — less pure as sentiment gauge."),
        ]
        for row_label, ticker, val, hist_key, is_eq, tip in pc_rows:
            lbl, col = pc_label_fn(val, is_eq)
            val_s    = f"{val:.3f}" if val is not None else "—"
            # 5-day trend from history
            hist_s = opt_hist.get(hist_key)
            if hist_s is not None and len(hist_s) >= 2:
                d5 = hist_s.tail(5).values
                trend_val  = float(d5[-1]) - float(d5[0])
                trend_txt  = f"{'↑' if trend_val > 0.01 else '↓' if trend_val < -0.01 else '→'} {trend_val:+.3f} vs 5d ago"
                trend_col  = "#f87171" if trend_val > 0.02 else "#4ade80" if trend_val < -0.02 else "#94a3b8"
                # Mini sparkline as inline bars
                mn, mx = min(d5), max(d5)
                rng = mx - mn if mx != mn else 1
                bars = "".join(
                    f'<div style="height:{max(4, int((v-mn)/rng*28))}px;width:6px;background:{col};'
                    f'border-radius:1px;align-self:flex-end;opacity:.7"></div>'
                    for v in d5
                )
                sparkline = (
                    f'<div style="display:flex;align-items:flex-end;gap:2px;height:28px;'
                    f'margin-top:4px">{bars}</div>'
                )
            else:
                trend_txt  = "—"
                trend_col  = "#4a6080"
            ...
            pc_html_str += (
                ...
                f'<span style="color:#dde8f5;font-size:.78rem;font-weight:600">{row_label}</span>'
                f'<span style="color:#4a6080;font-family:{MONO};font-size:.6rem">{ticker}</span>'
                f'<span style="color:#22d3ee;font-family:{MONO};font-size:1.05rem;font-weight:700">{val_s}</span></div>'
                f'<div style="color:{col};font-family:{MONO};font-size:.66rem;font-weight:700;margin-top:2px">{lbl}</div>'
                f'<div style="display:flex;justify-content:space-between;align-items:center;margin-top:3px">'
                f'<div style="color:{trend_col};font-family:{MONO};font-size:.62rem">{trend_txt}</div>'
                f'{sparkline}</div>'
                f'<div style="color:#3a5a78;font-size:.6rem;margin-top:3px;line-height:1.4">{tip}</div>'
                f'</div>'
            )
        pc_html_str += (
            f'<div style="margin-top:7px;font-family:{MONO};font-size:.58rem;color:#2a4a6a">'
            f'Source: CBOE via yfinance · Refreshed every 15 min · '
            f'Equity P/C is most reliable sentiment gauge for retail traders</div>'
            f'</div>'
        )
    ...
    with op3:
        ...
        if breadth:
            ...
            st.markdown(
                ...
                f'Coverage: {n} of 503 S&P 500 stocks'
                f'{" (full index)" if breadth.get("is_full") else " (partial — full loads in background)"}'
                f'</div></div>',
                unsafe_allow_html=True)
        else:
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    for i, (market, info) in enumerate(session_status.items()):
        with sess_cols[i]:
            is_open = info["status"] in ("OPEN", "LUNCH")
            ...
            st.markdown(
                ...
                f'color:{info["color"] if is_open else "#4a6a8a"}">{info["status"] if is_open else "CLOSED"}</span></div>'
